...
IS
    CURSOR cur_dev IS
    SELECT d.id, d.description, d.type, d.price, d.ip_address, d.buying_date, d.guaranty_expiration_date FROM DEVICE d, NETWORK net
    WHERE d.id_network = net.id;
    v_dev cur_dev%rowtype;

    CURSOR cur_per IS
    SELECT p.id, p.description, p.type, p.price, p.buying_date FROM PERIPHERAL p, DEVICE d, NETWORK net
    WHERE d.id = p.id_device
    AND d.id_network = net.id;
    v_per cur_perm%rowtype;

    CURSOR cur_vm IS
    SELECT  vm.id, vm.description FROM VM vm, DEVICE d, NETWORK net
    WHERE d.id = vm.id_device
    AND d.id_network = net.id;
    v_vm cur_vm%rowtype;
